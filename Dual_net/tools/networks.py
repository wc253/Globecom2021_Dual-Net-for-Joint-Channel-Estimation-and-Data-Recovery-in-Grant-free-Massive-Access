...
def build_LISTA(prob,L,nd,T,initial_lambda=.1,untied=False):
    """
    ...
    B1 = A1 / (1.01 * la.norm(A1,2)**2)
    B1_ =  tf.Variable(B1,dtype=tf.float32,name='B1_0')
    S1_ = tf.Variable( np.identity(N1) -np.transpose( np.matmul(B1.T,A1)),dtype=tf.float32,name='S1_0')
    By1_ = tf.matmul(prob.y1_ ,tf.reshape(tf.tile(B1_,[L,1]),(L,M1,N1) ))   
    
    layers.append( ('Linear',By_,By1_,None) )
    ...
    xhat_,xhat1_ = eta( By_,By1_, lam0_,L,N,nd)
    
    layers.append( ('LISTA T=1',xhat_, xhat1_,(lam0_,) ) )
    
    for t in range(1,T):
        lam_ = tf.Variable( initial_lambda,name='lam_{0}'.format(t) )
        xhat_, xhat1_ = eta( tf.matmul(S_,xhat_) + By_, tf.matmul(xhat1_,tf.reshape(tf.tile(S1_,[L,1]),(L,N1,N1) ) ) + By1_, lam_,L,N,nd )
        layers.append( ('LISTA T='+str(t+1),xhat_,xhat1_,(lam_,)) )
    return layers
