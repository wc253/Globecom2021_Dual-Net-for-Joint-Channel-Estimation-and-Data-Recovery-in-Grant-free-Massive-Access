...
def joint_threshold(r_,r1_,lam_,L,N,nd):
    "implement a soft threshold function y=sign(r)*max(0,abs(r)-lam)"
    r_combin=tf.concat([tf.reshape(tf.transpose(r_),(L,1,N)),r1_],axis=1)
    r_combin_thre= r_combin* tf.reshape(tf.maximum(1 - lam_/(tf.norm(r_combin,axis=1)), 0),(L,1,N))
    rest=tf.split(r_combin_thre,[1,nd], axis=1)
    r_thre=tf.transpose(tf.reshape(rest[0],(L,N)))
    r1_thre=rest[1]
    return r_thre,r1_thre
