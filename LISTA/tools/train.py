...
def setup_training(layer_info,prob, trinit=1e-3,refinements=(.5,.1,.01),final_refine=None ):
    ...
    training_stages=[]
    for name,xhat_,var_list in [layer_info[len(layer_info)-1]]:
...
def do_training(training_stages,prob,savefile,ivl=100,maxit=15000,better_wait=5000):
    """
    ...
    for name,xhat_,loss_,nmse_,train_,var_list in training_stages:
        ...
        for i in range(maxit+1):
            if i%ivl == 0:
                nmse = sess.run(nmse_,feed_dict={prob.y_:yval,prob.x_:xval})
                if np.isnan(nmse):
